...
describe('AuthContext', () => {
  ...
  it('fetches user on mount when token exists in sessionStorage', async () => {
    const token = 'mock-jwt-token'
  ...
  it('handles login successfully with remember me', async () => {
  ...
  it('handles login successfully without remember me', async () => {
    mockAxios.get.mockResolvedValueOnce({ data: {} }) // CSRF cookie
    mockAxios.post.mockResolvedValueOnce({
      data: {
        user: mockUsers.admin,
  ...
  it('handles multiple token sources correctly', async () => {
    // Test localStorage token takes precedence
    ...
    expect(result.current.user).toEqual(mockUsers.admin)
    expect(axios.defaults.headers.common['Authorization']).toBe('Bearer local-token')
  })
